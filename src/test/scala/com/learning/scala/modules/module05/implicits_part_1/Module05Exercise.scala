...
import com.learning.scala.modules.testutil.StopOnFirstFailure
 ...
 */
class Module05Exercise extends FunSuite with Matchers with StopOnFirstFailure with SeveredStackTraces {
    ...
    implicit object ReversableString extends Reversable[String] {
        override def reverse(x: String): String = {
            x.reverse
        }
    }
    ...
    implicit object ReversableInt extends Reversable[Int] {
        override def reverse(x: Int): Int = {
    ...
    implicit def reversableList[T: Reversable]: Reversable[List[T]] = { (xs: List[T]) =>
        val innerReverser: Reversable[T] = implicitly[Reversable[T]]
    ...
    def interceptException[T](fn: => Unit)(implicit ct: ClassTag[T]): Unit = {
        try {
            fn
            fail("Expected exception %s but didn't get it".format(ct.runtimeClass.getName))
        } catch {
            case NonFatal(th) if th.getClass == ct.runtimeClass =>
            case NonFatal(th) =>
...
//            }
            ...
            def eventually(f: => Unit)(implicit maxTries: MaxTries, interval: Interval): Unit = {
                eventuallyWith(maxTries.value, interval.value)(f)
            }
